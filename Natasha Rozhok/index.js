let d3 = require('d3')
let SVGCatmullRomSpline = require('svg-catmull-rom-spline')
...
let pathObj = d3.path()
pathObj.moveTo(pts[0][0], pts[0][1])
pts.shift()
pts.forEach(p => {
  pathObj.lineTo(...p)
})
...
//     let bezierCortages = SVGCatmullRomSpline.toPoints(points)
...
//     let bezierCortages = SVGCatmullRomSpline.toPoints(points)
//     console.log('bezierCortages',bezierCortages)
...
//     // let attribute = SVGCatmullRomSpline.toPath(points, tolerance, highestQuality);
//     // return attribute
//   }
...
// console.log(pointsToSplinePath(points,true))
...
//   // const rules = grammar({
//   //   "start": `<rect x=-10000 y=-10000 width=20000 height=20000 fill=${palette[4]} />{art}`,
//   //   'art': ['{g}', '{g} {start}'],
//   //   'g': ['{kebab}'.repeat(8), '{kebab} {g}'],
//   //   'kebab': [`<g transform='translate(r-200:201 r-200:201)'>{kebabLine}{kebabCircles}</g>`],
//   //   'kebabLine': `<line x1=0 x2=0 y1="r-220:0" y2="r:220" stroke-width='r1:3' {stroke} />`,
//   //   'kebabCircles': ['{circles}'],
//   //   'circles': [`{circle}`.repeat(4) + `{circles}`,
//   //     `{circle}`.repeat(4)],
//   //   'circle': [`<circle cx='0' cy='0' r='r80' stroke-width='r1:3:1' {fill} {stroke} />`,
//   //             ``
//   //     ],
//   //   // 'fill': [`fill=`+palette[0], `fill=`+palette[1], ``, ``, ``, ``, ``, ``, ``],
//   //   'fill': [``],
//   //   'stroke': [`stroke=`+palette[2], `stroke=`+palette[3]],
//   //   // "AB": ['A', 'B'],
//   // })
...
//     switch (nt) {
//       case 's':
//         if (Math.random() < .5) r('circle')
//         else r('sym')
//       case 'sym':
//         let id = Math.random()
//         let steps = 2 ** (Math.floor(Math.random() * 4) + 1)
//         let dist = 30 + 80 * Math.random()
//         html += `<circle id=${id} cx='0' cy='0' r='${Math.random() * 100}' />`
//         for (let a = 0; a < 360; a += 360 / steps) {
//           html += `<g transform='rotate(${a}) translate(${dist} 0)'><use href='#${id}'></g>`
//         }
//         break
//       case 'circle':
//         html += `<circle cx='0' cy='0' r='${Math.random() * 200}' />`
//         break
//     }
//   }
...
//   html = ''
//   for (let i = 0; i < 10; i++) {
//     let sw = Math.random() ** 1 * 300
//     let d1 = Math.random() * 4 / sw + 1
//     let d2 = Math.random() * 10 + 10
//     html += `<circle cx='0' cy='0' r='40' stroke-width='${sw}' stroke-dasharray='${d1} ${d2}'/>`
//   }
