...
r.draw()
document.querySelector('svg').setAttribute('viewBox', '0 0 3 3')
...
function a(i = 0, x = 0, y = 0, seed = rnd() * 1e7) {
  randomSeed(seed * 1000)
  let size = 1 / (3 ** i)
  if (i > depth || rnd() > .3 && i > 2) {
    return [{
      x: x, y: y, size: size, color: palette[Math.floor(palette.length*rnd())]
    }]
  }
  else {
    i++
...
function mouseClicked() {
  ...
  data.forEach(d => {
    let side = d.size * 3
    r.rect(d.x, d.y, side, side).fill(d.color).strokeWidth(1/winSize).stroke(d.color)
    // r.rect(d.x - eps, d.y - eps, side + 2 * eps, side + 2 * eps).fill(d.color).stroke(false)
  })
  r.draw();
  document.querySelector('svg').removeAttribute('xmlns:xlink')
}

window.addEventListener('resize', windowResized)
function windowResized() {
  winSize = Math.min(window.innerWidth, window.innerHeight)
  document.querySelector('svg').setAttribute('width', winSize)
  document.querySelector('svg').setAttribute('height', winSize)
}
