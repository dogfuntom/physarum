...
#define sabs(p) sqrt(abs(p)*abs(p)+.05)
...
float dist(V p) {
    ...
    for(int i = 0; i < BLOCKS_NUMBER_MAX; i++) {
        ...
        V pb = p;
        pb -= positions[i];
        pb.xz *= rot(rnd(float(i))*PI);//rot(rotations[i] * PI / 2.);
        pb.y+=sin(pb.z*50.*rnd(float(i-1)))*.1*rnd(float(i));
        ...
        if(types[i] == 0 || types[i] == 3 || types[i] == 4 || types[i] == 5 || types[i] == 6 || types[i] == 7) {
            // float c=length(pb.xy)-.29;
            // float b = box(pb, sizes[i] - 2. * (cornerR + gap));
            block = length(pb-vec3(0,0,clamp(pb.z,-sizes[i].z/2.+.5,sizes[i].z/2.-.5)))-.6;
