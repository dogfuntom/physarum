...
function a(i = 0, x = 0, y = 0, seed = random() * 1e7) {
  ...
  if (i > depth || random() > .3 && i > 2) {
    return [{
      x: x, y: y, size: size, color: random(
...
function mouseClicked() {
  seed = 56885985//Math.floor(1e8*Math.random())
  console.log(seed)
  let filename = `${seed}.png`
palette = palettes[Math.floor(random()*palettes.length)]
depth = Math.floor(random()*3+3)
let data = a().flat(8)

data.sort((a,b)=>Math.hypot(a.y-1.5,a.x-1.5)-Math.hypot(b.y-1.5,b.x-1.5))
for(let t=1;t<140;t++)
  data.forEach(d => {
    g.fill(...color(d.color).levels.slice(0,3).map(v=>v*t/100*1e-1/d.size))
    let sizeCell =d.size * 3 + 1 / g.width
    let sizeDraw=sizeCell-t*1e-3
    let x=d.x+sizeCell/2
    let y = d.y+sizeCell/2
    let angle=atan2(y-1.5,x-1.5)
    let dx = cos(angle)*t*2e-3*Math.hypot(y-1.5,x-1.5)
    let dy = sin(angle)*t*2e-3*Math.hypot(y-1.5,x-1.5)
    // let dx = +t*1e-3
    // let dy = +t*1e-3
    g.rect(x-sizeDraw/2+dx, y-sizeDraw/2+dy, max(0,sizeDraw))
  })
  g.save(filename)
}
