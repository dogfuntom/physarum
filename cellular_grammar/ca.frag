...
void main() {
    ...
    float n0 = step(.5, texture2D(tex, fract(uv)).r);
    float n1 = step(.5, texture2D(tex, fract(uv - vec2(size, 0))).r);
    float n2 = step(.5, texture2D(tex, fract(uv - vec2(0, size))).r);
    float n3 = step(.5, texture2D(tex, fract(uv + vec2(size, 0))).r);
    float n4 = step(.5, texture2D(tex, fract(uv + vec2(0, size))).r);
